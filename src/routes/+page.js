export const load = async ({ fetch }) => {
	...
	const fetchPopular = await fetch('https://api.consumet.org/meta/anilist/popular?format=TV&perPage=16');
	const fetchPopularData = await fetchPopular.json();
	let popular = fetchPopularData.results;

	const fetchRecentAiring = await fetch(
		'https://api.consumet.org/meta/anilist/recent-episodes?format=TV&perPage=16'
	);
	...
	const fetchActionAdventure = await fetch(
		'https://api.consumet.org/meta/anilist/advanced-search?genre=action&sort=[%22POPULARITY_DESC%22]&format=TV&perPage=16'
	);
	...
	return {
		...
		actionAdventureAnime: fetchActionAdventureData.results,
		adventureAnime: fetchActionAdventureData.results
	};
};
